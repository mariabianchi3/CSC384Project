...
class WaypointMapState(StateSpace):
    ...
    def successors(self):
        ...
        for direction in (UP, RIGHT, DOWN, LEFT):
            ...
            new_state = WaypointMapState(direction.name, self.gval + transition_cost, self, self.width, self.height, new_location, self.des_pos, self.POI, self.obstacles)
    ...
    def state_string(self):
        ...
        # Thrown down the POI's
        for key in list(self.POI.keys()):
            for coord in list(self.POI[key]):
                map[coord[1]][coord[0]] = key
...
'''
PROBLEMS = (
    WaypointMapState("START", 0, None, 5, 5, # Dimensions
                     (0,0), # Initial Position 
                     (4,4), # Desired Position 
                     {'A': [(1,1),(3,1)], 
                      'B': [(1,3)]}, # Dict of POI... Needs thinking about...
                     frozenset(((2,4),(3,3))) # Obstacles
                    ),
    WaypointMapState("START", 0, None, 6, 6, # Dimensions
                     (0,0), # Initial Position 
                     (5,5), # Desired Position 
                     {'A': [(1,1),(3,1)], 
                      'B': [(1,3)]}, # Dict of POI... Needs thinking about...
                     frozenset(((3,3),(3,4),(3,5),(4,2))) # Obstacles
                    ),
    WaypointMapState("START", 0, None, 6, 6, # Dimensions
                     (0,0), # Initial Position 
                     (5,5), # Desired Position 
                     {'A': [(1,1),(3,1)], 
                      'B': [(1,3)]}, # Dict of POI... Needs thinking about...
                     frozenset(((3,3),(3,4),(3,5),(4,2),(5,2))) # Obstacles
                    )
